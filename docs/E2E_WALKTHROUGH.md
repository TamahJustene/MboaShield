# MboaShield — End-to-end walkthrough (v1.9.0)
...
|---|---|---|
| Version | `1.9.0` via `/health` | Same image + env |
...
3. Or manually:
   - **Text** ? `POST /api/v1/check/text` (rumour + sources)
   - **Identity** ? `POST /api/v1/check/impersonation` (17 institutions)
   - **Audio** ? sample WAV ? `POST /api/v1/check/audio`
   - **Image** ? sample JPG ? `POST /api/v1/check/media`
   - **Ambassadors** ? lessons ? `POST /api/v1/ambassadors/complete`
4. **AI Case** panel ? `POST /api/v1/intelligence/analyze` — fused trust score; always `certainty: "none"`.
...
| Government | incidents, workflow | Human gates |
...
Citizen check or report
    ...
    ? analyst_review (human)
    ? optional institution_review
    ? decision (human) — NOT from AI alone
...
4. Public advisories require workflow state `public_advisory` after human `decision`.
